...
#:
#: `ctrlrun.transport` is the exception that is core and stdlib: `import ctrlrun` deliberately
#: does not import it (SPEC-v0.7 §2.8, T228), so its names are not in `ctrlrun.__all__` and the
#: five of them are listed here instead. A reference that named `NotExecuted` and not the module
#: that raises it correctly would leave a reader with the one decision and none of the help.
EXTRA_NAMES: tuple[tuple[str, str], ...] = (
    ("ctrlrun.transport", "Transport"),
    ("ctrlrun.transport", "effect_state"),
    ("ctrlrun.transport", "urlopen"),
    ("ctrlrun.transport", "HTTPConnection"),
    ("ctrlrun.transport", "HTTPSConnection"),
...
def _load():  # noqa: ANN202 - griffe's types are the return value
    ...
    return griffe.load("ctrlrun", search_paths=[str(CORE_ROOT / "src")], docstring_parser="google")

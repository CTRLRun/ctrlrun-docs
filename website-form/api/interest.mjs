...
// Same allowlist as the review endpoint: these two forms sit on the same pages.
...
export function validate(input) {
  ...
  const intent = field('intent', 32);
  const shape = Object.hasOwn(INTENTS, intent) ? INTENTS[intent] : undefined;
  ...
  return { intent, shape, email, company, message, agents, requestId };
}
...
export function createHandler({ env = process.env, fetcher = fetch, now = Date.now, rateStore = requests } = {}) {
  return async function handler(req, res) {
    ...
    if (!env.RESEND_API_KEY) return res.status(503).json({ error: 'This form is temporarily unavailable. Please email contact@arpanghoshal.com.' });

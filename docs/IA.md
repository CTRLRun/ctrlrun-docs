...
- Paths are root-relative on the site: `docs/concepts/effect-keys` is `docs/docs/concepts/effect-keys.mdx`.
...
ends with Next links. Every claim on a Concepts page has a `docs/docs/CLAIMS.md` row.
...
| Path | Purpose | Query |
|---|---|---|
| `security/threat-model` | What CTRLRun defends against, what it does not, and the fail-closed rules that follow; renders `docs/docs/THREAT_MODEL.md`. | *ctrlrun threat model* |
...
| Path | Purpose | Query |
|---|---|---|
| `architecture/overview` | The boundary CTRLRun owns, the canonical flow (normalize · decide · approve · reserve · execute · record), the module map; renders `docs/docs/ARCHITECTURE.md`. | *ctrlrun architecture* |

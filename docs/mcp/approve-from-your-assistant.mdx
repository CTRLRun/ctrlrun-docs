...
It opens no socket. The approver is the **OS login of the process**, which the client that
launched it cannot choose: not the environment it set, not anything it sends, but what the
kernel says the real uid is. Answers are recorded as `mcp-operator:<your login>` with the issuer
`os-login:<host>`, so the evidence says how the answer arrived. A login carries no roles and no
expiry, so a control that names an `approver_role` refuses over stdio. It takes no header, JWT
or origin flag; each is refused by name, because a flag that cannot take effect is a flag you
would believe took effect.

...
the remote answered once already and is holding the exchange, so nothing on the second leg can
say the remote did nothing. `ctrlrun.transport` refuses to claim `NotExecuted` on a resumed run,

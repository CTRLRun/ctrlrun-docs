...
One declared key set to the wrong type is enough to damage a row past reading, and until 0.11
**one `UPDATE` blinded every reader at once**. Now the row is named where it sits:

```
seq 2  ctr_b9db23088ad0…  UNREADABLE  this row could not be read (InvalidArgument)
```

`--verify-chain` reports `content_altered` there, and `stats` counts the rest as
`unreadable receipts` rather than dropping them from a total.
...
Erasing the **end** of the log is two statements and this command reports the store intact
afterwards, because the head that would catch it is a row in the same database.
[Anchoring](/docs/production/anchoring) puts that head out of the writer's reach.
...
**Verified by** `T510` and `T511` for the readers that no longer blind, `T164` — six tamper cases, each asserted on the **name** and the `seq` and not

...
//
// It runs the page's own Python, read out of try-it.js: the PLAYGROUND module, driven
...
function pythonIn(name) {
  ...
  if (!body) throw new Error(`try-it.js: no ${name} array — the page's Python moved`);
  return JSON.parse(`[${body[1]}]`).join("\n");
}
const PLAYGROUND = pythonIn("PLAYGROUND");
const PROGRAM = pythonIn("PROGRAM");
console.log("programs read from try-it.js:", PLAYGROUND.split("\n").length, "+", PROGRAM.split("\n").length, "lines");
